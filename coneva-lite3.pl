...
# User inputs 
my ($in_rr, $in_pdb_file, $out_file, $atom_type, $d_threshold, $min_seq_sep, $max_seq_sep, $jaccard_selct, $flag_show_all);
...
foreach my $rr (sort @rr_list){
	#printf $fh "%-30s", "".basename($rr);
	printf $fh "%s,", "".basename($rr);

	foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
		my %this_rr = %{$all_rr{"".basename($rr)." ".$top}};
		#printf $fh "%-10s", calc_precision(\%this_rr);
		printf $fh "%s,", calc_precision(\%this_rr);
	}
	print $fh "\n";
}
